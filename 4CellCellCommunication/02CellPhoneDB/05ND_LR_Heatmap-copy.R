...
means <- fread("/Users/coellearth/Desktop/Mammary_Gland_Diet_Project/4CellCellCommunication/02CellPhoneDB/cpdb_outputs/statistical_analysis_ND/means.txt")
pvals <- fread("/Users/coellearth/Desktop/Mammary_Gland_Diet_Project/4CellCellCommunication/02CellPhoneDB/cpdb_outputs/statistical_analysis_ND/pvalues.txt")
sig   <- fread("/Users/coellearth/Desktop/Mammary_Gland_Diet_Project/4CellCellCommunication/02CellPhoneDB/cpdb_outputs/statistical_analysis_ND/significant_means.txt")
...
unique(ND@meta.data$subcluster)

# 8. 定义顺序 & 调色板
cell_order  <- unique(ND@meta.data$subcluster)
n <- length(cell_order)
max_paired <- brewer.pal.info["Paired", "maxcolors"]
base_cols  <- brewer.pal(max_paired, "Paired")
paired_cols <- if (n <= max_paired) {
  brewer.pal(n, "Paired")
} else {

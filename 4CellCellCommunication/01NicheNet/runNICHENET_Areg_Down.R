...
ligand_expr <- ligand_expr %>%
  filter(grepl("^Areg", features.plot))
str(ligand_expr)
...
top_row    <- p2 + p1 + plot_layout(ncol = 2, widths = c(2, 13))
bottom_row <- plot_spacer() + p3 + plot_layout(ncol = 2, widths = c(5, 5.8))

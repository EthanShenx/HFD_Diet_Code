...
for (cell in cell_types) {
  ...
  deg$regulation[deg$p_val_adj < 0.05 & deg$avg_log2FC > 0.5] <- "Up"
  deg$regulation[deg$p_val_adj < 0.05 & deg$avg_log2FC < -0.5] <- "Down"
  
  safe_cell <- gsub("[/\\\\]", "_", cell)
  out_file  <- file.path(base_dir, paste0(safe_cell, "_05_DEGs.csv"))
...
write.csv(all_degs,
          file      = "DEG_results/All_CellTypes_Combined_05_DEGs.csv",
          row.names = TRUE)

save(deg_list, cell_types, file = "02-01-envVariables_05.RData")

...
tmp2 <- meta %>%
  dplyr::count(orig.ident, cell_type, name = "count") %>%
...
n_ct <- length(unique(tmp2$cell_type))
base_n <- min(max(3, n_ct), 12)
pal <- colorRampPalette(brewer.pal(base_n, "Paired"))(n_ct)
colors <- setNames(pal, sort(unique(tmp2$cell_type)))
...
counts_by <- tmp2 %>%
  dplyr::select(orig.ident, cell_type, count) %>%
...
test_list <- purrr::pmap(
  list(wide_ct$cell_type, wide_ct$ND, wide_ct$HFD),
...
write.csv(prop_test_df, "/Users/coellearth/Desktop/HFD_Paper/f-Fig1scLandscpae/cellComposition/cell_prop_fisher_res_not_subdivided.csv", row.names = FALSE)
...
# (可选) 保证 cell_type 的顺序稳定
tmp2 <- tmp2 %>%
  dplyr::mutate(cell_type = factor(cell_type, levels = sort(unique(cell_type))))

p <- ggplot(
  data = tmp2,
  aes(x = orig.ident, y = rate, fill = cell_type)
) +
...
ggplot(tmp2, aes(x = orig.ident, y = count, fill = cell_type)) +

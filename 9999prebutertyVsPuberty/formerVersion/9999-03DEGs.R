...
cell_types <- as.vector(unique(allStages@meta.data$cell_type_refined))
...
for (cell in cell_types) {
  sub.obj <- subset(allStages, subset = cell_type_refined == cell)
  Idents(sub.obj) <- "orig.ident"
  ...
  write.csv(
    deg,
    file      = paste0("DEG_results/", cell, "_DEGs_refined.csv"),
    row.names = TRUE
  )
...
write.csv(
  all_degs,
  file      = "DEG_results/allStages_refinedCellTypes_Combined_DEGs.csv",
  row.names = TRUE
)

save(deg_list, cell_types, file = "9999-02-envVariables_refined.RData")
